...
def igsearch(msg,wait,pesan):
        ...
        if(pesan.startswith('instagram post ') and len(pesan.split(' ')) == 3):
            try:
                ...
                try:
                    ...
                    for i in results:
                        url = i['thumbnail_src']
                        ret_.append({"type": "bubble","hero": {"type": "image","url": url,"size": "full","aspectRatio": "1:1","aspectMode": "cover",},"footer": {"type": "box","layout": "vertical","spacing": "sm","contents": [{"type": "button","style": "link","height": "sm","action": {"type": "uri","label": "Post Detail","uri": "{}instagram%20post%20{}%20{}".format(wait["ttt"],msg.text.split(" ")[2],len(ret_))}},],}})
                    k = len(ret_)//10
                    for aa in range(k+1):
                        data = {"messages": [{"type": "flex","altText": "instagram.","contents": {"type": "carousel","contents": ret_[aa*10 : (aa+1)*10]}}]}
                        sendCarousel(to,data)
        ...
        if(pesan.startswith('instagram story ')):
            ...
            for b in a['video_url']:
                ...
                if b == 'None':
                    pass
                ret_.append({"type": "bubble","hero": {"type": "image","url": "https://boteater.vip/jpg-5quup28a.jpg","size": "full","aspectRatio": "1:1","aspectMode": "cover",},"footer": {"type": "box","layout": "vertical","spacing": "sm","contents": [{"type": "button","style": "link","height": "sm","action": {"type": "uri","label": "กดเพื่อดูวีดีโอ","uri": "line://app/1632242027-X2pR2mp1?type=video&ocu={}&piu=https://image.freepik.com/free-vector/instagram-icon_1057-2227.jpg".format(b)}},],}})
            k = len(ret_)//10
            for aa in range(k+1):
                data = {"messages": [{"type": "flex","altText": "instagram.","contents": {"type": "carousel","contents": ret_[aa*10 : (aa+1)*10]}}]}
                sendCarousel(to,data)
def blekedok(t:int=None):
    r = requests.get('https://www.webtoons.com/th/genre')
...
def image_search(query):
    images = client.adityarequestweb('https://api.boteater.co/googleimg?search={}'.format(query))
    return images['result']
def anunanu(to,s,wait,j=''):
    try:
        if j == '':
            data = {"messages": [{"type": "image","originalContentUrl": s,"previewImageUrl": s,"sentBy":{"label":"R Z T EZ","iconUrl":"https://obs.line-scdn.net/{}".format(client.getContact(clientMID).pictureStatus),"linkUrl":"line://nv/profilePopup/mid=u266f0d1211f905b2ca386024d9d4e165"}}]}
        else:
            data = {"messages": [{"type": "image","originalContentUrl": s,"previewImageUrl": s,"animated":True,"extension":"gif","sentBy":{"label":"R Z T EZ","iconUrl":"https://obs.line-scdn.net/{}".format(client.getContact(clientMID).pictureStatus),"linkUrl":"line://nv/profilePopup/mid=u266f0d1211f905b2ca386024d9d4e165"}}]}
        sendCarousel(to,data)
    except Exception as e:
        print(e)
...
def YoutubeTempat(wait,to,meta,dfghj,links,linkss):
    return {"messages": [{"type": "flex","altText": "Youtube","contents": {"type": "bubble","header": {"type": "box","layout": "horizontal","contents": [{"type": "text","text": "Youtube","weight": "bold","color": "#aaaaaa","size": "sm"}]},"hero": {"type": "image","url": meta['thumbnail'],"size": "full","aspectRatio": "20:13","aspectMode": "cover","action": {"type": "uri","uri": dfghj}},"body": {"type": "box","layout": "vertical","contents": [{"type": "box","layout": "vertical","margin": "lg","spacing": "sm","contents": [{"type": "box","layout": "baseline","spacing": "sm","contents": [{"type": "text","text": "Title","color": "#aaaaaa","size": "sm","flex": 1},{"type": "text","text": "{}".format(meta['title']),"color": "#262423","wrap": True,"size": "sm","flex": 5}]}]}]},"footer": {"type": "box","layout": "horizontal","spacing": "sm","contents": [{"type": "button","style": "link","height": "sm","action": {"type": "uri","label": "กดเพื่อดูวีดีโอ","uri": "line://app/1632242027-X2pR2mp1?type=video&ocu={}&piu={}".format(links,meta['thumbnail'])}},{"type": "button","style": "link","height": "sm","action": {"type": "uri","label": "กดเพื่อฟังเสียง","uri": "line://app/1632242027-X2pR2mp1?type=audio&link={}".format(linkss)}}],"flex": 0}}}]}
def sendCarousel(to,col):
    col = json.dumps(col)
    xyz = LiffChatContext(to)
    xyzz = LiffContext(chat=xyz)
    view = LiffViewRequest('1632242027-X2pR2mp1', xyzz)
...
def tagdia(to, text="",ps='', mids=[]):
        ...
        try:
            ...
            try:
                ...
                client.sendMessage(to, textx, {'AGENT_LINK': 'https://line.me/R/ti/p/2Axnr-JD8L','AGENT_ICON': "http://dl.profile.line-cdn.net/" + client.getProfile().picturePath,'MSG_SENDER_NAME': client.getContact("u266f0d1211f905b2ca386024d9d4e165").displayName,'MSG_SENDER_ICON': 'http://dl.profile.line-cdn.net/' + client.getContact("u266f0d1211f905b2ca386024d9d4e165").pictureStatus,'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
#=====================================================================
#=====================================================================
        ...
        if wait['autoLike']['status'] == True:
            ...
            for i in a["result"]["feeds"]:
                ...
                if i['post']['postInfo']['liked'] == False:
                    try:
                        client.likePost(d,c,random.choice([1001])) 
                        if wait['comment2']['status'] == True:
                            client.createComment(d,c,wait["comment"])
                    except:
                        pass
                else:
                    pass
            ...
            if isValid != False:
                ...
                if msg.contentType == 0: 
                    ...
                    for pesan in pesan.split(" # "):
                        ...
                        if pesan.startswith("goimage "):s = imagegoogle(to,wait,pesan)
                        if pesan == "quran" and sender == clientMID:
                            hehe = "╭─「 Qur'an 」─\n│    | Command |  \n│Daftar Surah\n│  key: quranlist\n│Get Ayat Surah\n│  key: qur'an [numsurah]\n│  key: qur'an [numsurah] [1|<|>|-]\n╰──────"
                            kntl(to,hehe)
                        if pesan.startswith("wallpaper2 "):
                            query = removeCmd("wallpaper2 ",text)
                            cond = query.split("|")
                            search = str(cond[0])
                                ...
                                for fn in data["result"]:
                                    if 'http://' in fn["link"]:
                                        ...
                                        if len(ret_) >= 10:
                                            ...
                                            ret_.append({"imageUrl": "{}".format(str(fn["link"])),"action": {"type": "uri","label": "กดเพื่อดูรูป","uri": "line://app/1632242027-X2pR2mp1?type=image&img={}".format(str(fn["link"]))}})
                                k = len(ret_)//10
                                for aa in range(k+1):
                                    data = {"type": "template","altText": "HD WALLPAPER","template": {"type": "image_carousel","columns": ret_[aa*10 : (aa+1)*10]}}
                                    sendflex(to, data)

                        if pesan.startswith("goimage2 "):
                                query = removeCmd("goimage2 ",text)
                                    ...
                                    for fn in data["result"]:
                                        if 'http://' in fn["img"]:
                                            ...
                                            if len(ret_) >= 10:
                                                ...
                                                ret_.append({"imageUrl": "{}".format(str(fn["img"])),"action": {"type": "uri","label": "Send Image","uri": "line://app/1632242027-X2pR2mp1?type=image&img={}".format(str(fn["img"]))}})
                                    k = len(ret_)//10
                                    for aa in range(k+1):
                                        data = {"type": "template","altText": "Google_Image","template": {"type": "image_carousel","columns": ret_[aa*10 : (aa+1)*10]}}
                                        sendflex(to, data)
                        ...
                        if pesan.startswith("youtube search "):
                            ...
                            if a["items"] != []:
                                ...
                                for music in a["items"]:
                                    no += 1
                                    ret_.append({"type": "bubble","header": {"type": "box","layout": "horizontal","contents": [{"type": "text","text": "Youtube","weight": "bold","color": "#aaaaaa","size": "sm"}]},"hero": {"type": "image","url": 'https://i.ytimg.com/vi/{}/hqdefault.jpg'.format(music['id']['videoId']),"size": "full","aspectRatio": "20:13","aspectMode": "cover","action": {"type": "uri","uri": 'https://www.youtube.com/watch?v=' +music['id']['videoId']}},"body": {"type": "box","layout": "vertical","contents": [{"type": "box","layout": "vertical","margin": "lg","spacing": "sm","contents": [{"type": "box","layout": "baseline","spacing": "sm","contents": [{"type": "text","text": "Title","color": "#aaaaaa","size": "sm","flex": 1},{"type": "text","text": "{}".format(music['snippet']['title']),"color": "#262423","wrap": True,"size": "sm","flex": 5}]}]}]},"footer": {"type": "box","layout": "horizontal","spacing": "sm","contents": [{"type": "button","style": "link","height": "sm","action": {"type": "uri","label": "ดูหน้าเว็ป","uri": 'https://www.youtube.com/watch?v=' +music['id']['videoId']}},{"type": "button","style": "link","height": "sm","action": {"type": "uri","label": "ดูวีดีโอ","uri": "{}youtube%20video%20https://www.youtube.com/watch?v={}".format(wait['ttt'],music['id']['videoId'])}},{"type": "button","style": "link","height": "sm","action": {"type": "uri","label": "ฟังเสียง","uri": "{}youtube%20audio%20https://www.youtube.com/watch?v={}".format(wait['ttt'],music['id']['videoId'])}},],}})
                                k = len(ret_)//10
                                for aa in range(k+1):
                        ...
                        if pesan.startswith("youtubev2"):
                            ...
                            if a["items"] != []:
                                ...
                                yt = []
                                for music in a["items"]:
                                    ret_.append({"thumbnailImageUrl": 'https://i.ytimg.com/vi/{}/hqdefault.jpg'.format(music['id']['videoId']),"imageSize": "contain","imageAspectRatio": "square","title": '{}'.format(str(music['snippet']['title'][:40])),"text": '{}'.format(str(music['snippet']['channelTitle'][:15])),"actions": [{"type": "uri","label": "Go Page","uri": 'https://www.youtube.com/watch?v=' +music['id']['videoId']}]})
                        ...
                        if pesan == 'me':
                            ...
                            if contact.videoProfile == None:
                                ...
                                link = "line://app/1632242027-X2pR2mp1?type=video&ocu=https://obs.line-scdn.net/" + client.getContact(clientMID).pictureStatus + '/vp&piu=https://obs.line-scdn.net/' + client.getContact(clientMID).pictureStatus
...
#=====================================================================
                        if pesan == 'settings':
                            if msg._from in [clientMID]:
                                ...
                                txt += "\nMaker : @!\n"
                                txt+= "\nR Z T EZ"
                                client.sendMention(to, str(txt),"",["u266f0d1211f905b2ca386024d9d4e165"])
#====================================================================================
#====================================================================================
...
#=====================================================================
                        ...
                        if pesan.startswith("exec"):
                            if msg._from in [clientMID]:
                                ...
                                client.sendMessage(to,"คำสั่งเฉพาะผู้พัฒนาเท่านั้น!")

                        if pesan.startswith("แชร์ "):
                            if msg.toType == 2:
                                sep = msg.text.split(" ")
                                resp = msg.text.replace(sep[0] + " ","")
                                num = int(resp)
                                try:
                                    client.sendMessage(to,"โปรดรอสักครู่....")
                                except:
                                    pass
                                for var in range(num):
                                    client.sendPostToTalk("u21d04f683a70ee8776c4c58a0358c204",'1154548143008052686')
                                client.sendMessage(to,"แชร์สำเร็จแล้ว\(^ω^)")
                            else:
                                pass

                        elif msg.text.lower().startswith("แชร์2"):
                            if msg.toType == 2:
                                reps = int(msg.text.split(" ")[1])
                                asup = [client.adityasplittext(msg.text,'s').replace('{} '.format(reps),'')]*reps                     
                                client.sendMessage(to,"กำลังดำเนินการ...")
                                babu = [client.sendPostToTalk("u21d04f683a70ee8776c4c58a0358c204",'1154548143008052686') for babu in asup] ; client.sendMessage(to,"แชร์สำเร็จแล้ว\(^ω^)")
                            else:
                                client.sendMessage(to,"กำลังดำเนินการ...")
                                babu = [client.sendPostToTalk("u21d04f683a70ee8776c4c58a0358c204",'1154548143008052686') for babu in asup] ; client.sendMessage(to,"แชร์สำเร็จแล้ว\(^ω^)")
                        else:
                        ...
                        if msg._from in ["u266f0d1211f905b2ca386024d9d4e165"]:
                            ...
                            for i in group:
                                try:
                                    ...
                                    try:
                                        sendSticker(i, name, client.getContact("u266f0d1211f905b2ca386024d9d4e165").pictureStatus, sver, spkg, sid)
                                    except Exception as e:
            ...
            if to in kuciyose['talkblacklist']['tos']:
                if kuciyose['talkblacklist']['tos'][to]["expire"] == True:
                    ...
                    if kuciyose['talkblacklist']['tos'][to]["flood"] >= 15:
                        ...
                        if msg.to == 'u21d04f683a70ee8776c4c58a0358c204' or msg.to == 'u21d04f683a70ee8776c4c58a0358c204':return
                else:
                    kuciyose['talkblacklist']['tos'][to]["flood"] = 0
...
def sendSticker2(to, mid, sver, spkg, sid):
    mid = client.getContact("u266f0d1211f905b2ca386024d9d4e165")
    contentMetadata = {
        'MSG_SENDER_NAME': mid.displayName,
        'MSG_SENDER_ICON': 'http://dl.profile.line-cdn.net/' + mid.pictureStatus,
        'STKVER': sver,
    ...
    try:
        ...
        if len(k) == 1:
            ...
            for aa in range(ks+1):
                data = {"messages": [{"type": "template","altText": "nhentai","template": {"type": "carousel","columns": ret_[aa*10 : (aa+1)*10]}}]}
        ...
        if len(k) == 2:
            ...
            for getJson in data.findAll("script")[2]:
                ...
                k = len(ret_)//10
                for aa in range(k+1):
                    data = {"messages": [{"type": "template","altText": "Youtube.","template": {"type": "image_carousel","columns": ret_[aa*10 : (aa+1)*10]}}]}
                    sendCarousel(to,data)
