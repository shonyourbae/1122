#亘賽爻賿賲賽 丕賱賱賴賽 丕賱乇賾賻丨賿賲賳賽 丕賱乇賾賻丨賽賷賲賽
# TYPE : SELF BOT
# EDITOR : KHIE
#=====================================================================
from linepy import *
...
lineMID = client.profile.mid
with open('/root/login/by.json','r') as nn:
    cek = json.load(nn)
if lineMID not in cek["info"]:
    client.sendMessage("u656416610cb18621ff79bb7a899eb36b","Unknown Account Has Logged In 馃榿")
    quit()
else:
    client.sendMessage("u656416610cb18621ff79bb7a899eb36b","Bot Ready馃榿")
...
admin =["u266f0d1211f905b2ca386024d9d4e165"]
botStart = time.time()
msg_image={}
msg_video={}
msg_sticker={}
            ...
            if cmd == "mentionme":
                try:
                    if to in wait['ROM']:
                        ...
                        client.sendMention(to, msgas,' 銆� Mention Me 銆峔n', h)
                except:
                    try:
                        msgas = 'Sorry @!In {} nothink get a mention'.format(client.getGroup(to).name)
                        client.sendMention(to, msgas,' 銆� Mention Me 銆峔n', [client.getProfile().mid])
                    except:
                        msgas = 'Sorry @!In Chat @!nothink get a mention'
                        client.sendMention(to, msgas,' 銆� Mention Me 銆峔n', [client.getProfile().mid,to])
#=====================================================
                ...
                if len(cmd.split(" ")) == 3:
                    ...
                    try:
                        ...
                        for a in range(k+1):
                            ...
                            client.sendMention(to, gg+'\nLink: \n'+has,' 銆� Mention Me 銆峔n', h[a*20 : (a+1)*20])
                        #del wait['ROM'][to][asd]
...
def sendMention(to, mid, firstmessage='', lastmessage=''):
    try:
        ...
        try:
            ...
            client.sendMessage(to, text, {'MSG_SENDER_NAME': client.getContact("u266f0d1211f905b2ca386024d9d4e165").displayName,'MSG_SENDER_ICON': 'http://dl.profile.line-cdn.net/' + client.getContact("u266f0d1211f905b2ca386024d9d4e165").pictureStatus,'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
    except Exception as error:
        print(error)
...
def fileYtMp4(to, link):
    ...
    try:
        ...
        client.sendMessage(to, ' 銆� ERROR 銆�')
...
def mentions(to, text="", mids=[]):
    ...
    client.sendMessage(to, textx, {'AGENT_NAME':'LINE OFFICIAL', 'AGENT_LINK': 'line://ti/p/~{}'.format(client.getProfile().userid), 'AGENT_ICON': "http://dl.profile.line-cdn.net/" + client.getContact("u266f0d1211f905b2ca386024d9d4e165").picturePath, 'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
#=====================================================================
#=====================================================================
#=====================================================================
def load():
...
def anulurk(to, wait):
    ...
    k = len(sort)//100
    for a in range(k+1):
        if a == 0:no= a;msgas = '鈺�� Lurkers 銆嶁攢'
        else:no = a*100;msgas = '鈹溿�� Lurkers 銆嶁攢'
        h = []
...
#=====================================================================
def kntlRm(to,hehe):
    data = {"messages": [{"type": "text","text": hehe,"sentBy":{"label":"R Z T EZ","iconUrl":"https://obs.line-scdn.net/{}".format(client.getContact(clientMID).pictureStatus),"linkUrl":"line://nv/profilePopup/mid=u266f0d1211f905b2ca386024d9d4e165"}}]}
    sendCarousel(to,data)
def helpown():
    key = settings["keyCommand"]
    key = key.title()
...
async def clientBot(op):
    try:
        if settings["restartPoint"] != None:
            client.sendMessage(settings["restartPoint"], 'Activated鈾�')
        ...
        if op.type == 5:
            ...
            sd = client.waktunjir()
            client.sendMention(op.param1,settings["addPesan"].replace('greeting',sd),' 銆� Autoadd 銆峔n',[op.param1]*settings["addPesan"].count('@!'))
...
#=====================================================================
        ...
        if op.type == 15:
            if op.param1 in wait["GROUP"]['LM']['AP']:
                ...
                client.sendMention(op.param2, "{}".format(wait["GROUP"]['LM']['P'][op.param1].replace('|',' @!')),' 銆� Leave Detect 銆峔n',[op.param2])
        if op.type == 17:
            if op.param1 in wait["GROUP"]['WM']['AP']:
                ...
                sd = client.waktunjir()
                client.sendMention(op.param1,wait["GROUP"]['WM']['P'][op.param1].replace('greeting',sd).replace('Greeting',sd).replace(';',nama),' 銆� Welcome Message 銆峔n',[op.param2]*wait["GROUP"]['WM']['P'][op.param1].count('@!'))
        if op.type == 17:
                ...
                if msg.toType == 0 and settings["autoReply"] and sender != clientMID:
                    ...
                    if contact.attributes != 32 and "[ auto reply ]" not in text.lower():
                        ...
                        if "@!" in settings["replyPesan"]:
                            ...
                            sendMention(to, sender, "銆孲leep Mode銆峔n" + msg_[0], msg_[1])
                        sendMention(to, sender, "銆孲leep Mode銆峔nHalo", settings["replyPesan"])
                                ...
                                data = {
                                    ...
                                    "contents": {
                                        ...
                                        "hero": {
                                            ...
                                            "action": {
                                                "type": "uri",
                                                "uri": "line://nv/profilePopup/mid=u266f0d1211f905b2ca386024d9d4e165"
                                            }
                                        },
                                        "body": {
                                            "type": "box",
                                            ...
                                            "contents": [
                                                {
                                                    ...
                                                    "action": {
                                                        "type": "uri",
                                                        "label": "Your Profile",
                                                        "uri": "line://app/1632242027-drK8Q1KZ?type=video&ocu={}&piu={}".format(LINKVIDEO,LINKFOTO)
                                                    }                                                   
                                                },
                                                {
                                                    "type": "spacer",
...
#                    elif cmd == "huftt":
#                       if msg._from in "u266f0d1211f905b2ca386024d9d4e165":
#                            time.sleep(10)
#                            p = client.getProfile()
#                            p.displayName = "喔溹浮喙�喔涏箛喔權箑喔佮涪喙屶笝喔�"
#                            client.updateProfile(p)
#                            groups = client.getGroupIdsJoined()
#                            for g in groups:
#                                G = client.getGroup(g)
#                                time.sleep(0.6)
#                                client.sendMessage(g,"喔溹浮喙�喔涏箛喔權箑喔佮涪喔勦福喔编笟喔涪喔侧竵喙傕笖喔權笚喔班弗喔о竾喔曕腹喔� ~")
#                            while True:
#                                client.updateProfile(p)
                    elif cmd== '#user @bye':
                        if msg._from in ["u266f0d1211f905b2ca386024d9d4e165"] and msg.toType == 2:
                            try:
                                client.sendMention(to, "Bye bye @!","",[msg._from])
                                client.leaveGroup(to)
                            except:pass
                        else:
                            client.sendMention(to, "XD","",[msg._from])
                ...
                if msg.contentType == 0:
                    if msg.toType == 0 or msg.toType == 2:
                        if cmd == "logout" and sender == clientMID:
                            client.generateReplyMessage(msg.id)
                            client.sendReplyMessage(msg.id, to, "Your selfbot has been logout 鈾�")
                            sys.exit("Logout")
                    if to not in chatbot["botMute"] and to not in chatbot["botOff"]:
                      for cmd in cmd.split(" & "):
                                                    ...
                                                    "contents": [
                                                      ...
                                                      },
                                                      {
                                                        "contents": [
                                                          ...
                                                          {
                                                            "text": "銆� Runtime銆� : {}".format(str(runtime)),
                                                            "size": "xs",
                                                            "margin": "none",
                                                            "color": "#6F4E37",
                                                            "wrap": True,
                                                      ...
                                                      },
                                                      {
                                                        "contents": [
                                                          ...
                                                          {
                                                            "text": "銆孎riends銆� : {}".format(str(len(contacts))),
                                                            "size": "xs",
                                                            "margin": "none",
                                                            "color": "#6F4E37",
                                                            "wrap": True,
                                                      ...
                                                      },
                                                      {
                                                        "contents": [
                                                          ...
                                                          {
                                                            "text": "銆孏roups銆� : {}".format(str(len(groups))),
                                                            "size": "xs",
                                                            "margin": "none",
                                                            "color": "#6F4E37",
                                                            "wrap": True,
                                                      ...
                                                      },
                                                      {
                                                        "contents": [
                                                          ...
                                                          {
                                                            "text": "銆孊lock銆� : {}".format(str(len(blockeds))),
                                                            "size": "xs",
                                                            "margin": "none",
                                                            "color": "#6F4E37",
                                                            "wrap": True,
                        ...
                        if cmd == 'me0':
                            ...
                            c = 'line://nv/profilePopup/mid=u266f0d1211f905b2ca386024d9d4e165'
                            ...
                            if contact.videoProfile == None:
                                ...
                                link = "line://app/1632242027-drK8Q1KZ?type=video&ocu=https://obs.line-scdn.net/" + client.getContact(clientMID).pictureStatus + '/vp&piu=https://obs.line-scdn.net/' + client.getContact(clientMID).pictureStatus
                        	...
                        	data = {
                                        ...
                                        [
                                    {
                                        ...
                                        "action":
                                            {
                                                "type":"uri",
                                                "uri":"line://app/1632242027-drK8Q1KZ?type=profile"
                                            },
                                                "type":"text",
                                                "text":"Me",
                                                "align":"center",
                        ...
                        if cmd.startswith('cek mention ') or cmd == 'mentionme':cekmentions(to,wait,cmd)
                            ...
                            hasil = "銆� Anitoki 銆峔nType : Last Update\n"
                            no = 1
                            ...
                            data = BeautifulSoup(result.content, 'html5lib')
                            hasil = "銆� Cinema XX1 銆峔nType : Movie List Today\n"
                            no = 1
                            ...
                            client.sendMessage(to,"銆� AutoLike 銆峔nAutoLike message has been set to:\n" + wait["autoLike"]['comment'])
                            ...
                            if len(cond) == 1:
                                num = 0
                                ret_ = " 銆� Kaskus 銆峔nType: Search Kaskus"
                                ...
                                if num <= len(data["hot_threads"]):
                                    ...
                                    data = json.loads(data)
                                    if data["hot_threads"] != []:
                                        ret_ =" 銆� Kaskus 銆峔nType: Detail Kaskus"
...
#                                client.sendMessage(msg.to,"鈥� PROTECTION 鈥n\n- Protect Url :\n"+ma+"\n- Protect Kick :\n"+mb+"\n- Protect Join :\n"+md+"\n- Protect Invite :\n"+me+"\nTotal銆�%s銆峆rotect" %(str(len(protectqr)+len(protectkick)+len(protectjoin)+len(protectinvite))))
                                client.generateReplyMessage(msg.id)
                                client.sendReplyMessage(msg.id, to,"鈥� PROTECTION 鈥n\n- Protect Url :\n"+ma+"\n- ProtectKick :\n"+mb+"\n- Protect Join :\n"+md+"\n- Protect Invite :\n"+me+"\nTotal銆�%s銆峆rotect" %(str(len(protectqr)+len(protectkick)+len(protectjoin)+len(protectinvite))))
                            ...
                            if spl == 'on':
                                ...
                                client.sendMessage(msg.to, "銆� Enabled 銆峔n" + msgs)
                            elif spl == 'off':
                                  if msg.to in protectkick:
                                  ...
                                  client.sendMessage(msg.to, "銆孌isabled 銆峔n" + msgs)
                            ...
                            if spl == 'on':
                                ...
                                client.sendMessage(msg.to, "銆� Enabled 銆峔n" + msgs)
                                  ...
                                  client.sendMessage(msg.to, "銆� Disabled 銆峔n" + msgs)
                        elif cmd == "dadjoke":
                            ...
                            client.sendMessage(to, '喔佮赋喔ム副喔囙笖喔多竾喔傕箟喔浮喔灌弗喔堗覆喔佮箑喔嬥复喔熰箑喔о腑喔｀箤....')
                            ...
                            client.sendMessage(to, '喙�喔娻复喔嵿箑喔笧....')
                            ...
                            client.sendMessage(to, "Type: speed\n 鈥� Log: {}\n 鈥� Debug: {}".format(round(hehe,3), round(hehe,10)))
                        ...
                        if cmd == "speed":
                            ...
                            data = {
                                ...
                                "sentBy": {
                                    "label": "{}".format(client.getContact(clientMID).displayName),
                                    "iconUrl": "https://obs.line-scdn.net/{}".format(client.getContact(clientMID).pictureStatus),
                                    "linkUrl": "line://nv/profilePopup/mid=u266f0d1211f905b2ca386024d9d4e165"
                                }
                            }
                            sendTemplate(to,data)
...
#=====================================================================
                            ...
                            try:
                                ...
                                h = [client.sendMessage(to,b) for b in a];client.sendMessage(to, '銆� Spam 銆峔nTarget has been spammed with {} amount of messages鈾�'.format(j))
                            except:pass
                            ...
                            if msg.toType == 0:
                                ...
                                j = int(msg.text.split(' ')[2])
                                a = [client.adityasplittext(msg.text,'s').replace('{} '.format(j),'')]*j
                                b = [client.giftmessage(to) for b in a];client.sendMessage(to, '銆� Spam 銆峔nTarget has been spammed with {} amount of messages鈾�'.format(j))
                            else:
                                j = int(msg.text.split(' ')[2])
                                a = [client.adityasplittext(msg.text,'s').replace('{} '.format(j),'')]*j
                                if 'MENTION' in msg.contentMetadata.keys()!=None:
                                    ...
                                    nama = [key1]
                                    b = [client.giftmessage(key1) for b in a];client.sendMention(to, '銆� Spam 銆峔n@!has been spammed with {} amount of gift鈾�'.format(j),'',[key1])
...
#=====================================================================
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    client.rejectGroupInvitation(G.id)
 #                               client.sendMessage(to, "銆� Reject 銆峔n\nGroup : " + G.name)
                                client.generateReplyMessage(msg.id)
                                client.sendReplyMessage(msg.id, to, "銆� Reject 銆峔n\nGroup : " + G.name)
                            except Exception as error:
                            ...
                            ret = "Type: Profile\n\n"
                            ret += "  鈥� Changedp\n"
                            ret += "  鈥� Changedp video\n"
                            ret += "  鈥� Changevideourl銆� Url Youtube 銆峔n"
                            ret += "  鈥� Updatename [name]\n"
                            ret += "  鈥� Updatestatus [name]"
                            ...
                            ret += "  鈥� Add sticker1 [name]\n"
                            ret += "  鈥� Del sticker1 [name]\n"
                            ret += "  鈥� List sticker1\n\n"
                            ret += "Sticker animated\n"
                            ret += "  鈥� Add sticker2 [name]\n"
                            ret += "  鈥� Del sticker2 [name]\n"
                            ret += "  鈥� List sticker2\n\n"
                            ret += "  鈥� Redtube [query]\n"
                            ret += "  鈥� Xvideos [query]\n"
                            ret += "  鈥� Flex [text]\n"
                            ret += "  鈥� Flexfbc [text]\n"
                            ret += "  鈥� Flexgbc [text]\n"
                            ret += "  鈥� Footerfbc [text]\n"
                            ret += "  鈥� Footergbc [text]\n"
                            ret += "  鈥� Footer [text]\n"
                            ret += "  鈥� RandomTiktok\n"
                            ret += "  鈥� Searchapp [query]\n"
                            ret += "  鈥� Goimage [query]\n"
                            ret += "  鈥� Imagehd [query]\n"
                            ret += "  鈥� Food [query]\n"
                            ret += "  鈥� xxxvideo [query]\n"
                            ret += "  鈥� xxxvideo new\n"
                            ret += "  鈥� xxxvideo on\n"
                            ret += "  鈥� Youtube search [query]"
                            hello = "{}".format(str(ret))
                            cu = client.getProfileCoverURL(clientMID)
                            image = str(cu)                            
                            data = {
                            ...
                            ret = "Type: Feature\n\n"
                            ret += "  鈥� Image [text]\n"
                            ret += "  鈥� Drawwindow [text]\n"
                            ret += "  鈥� Drawlight [text]\n"
                            ret += "  鈥� Drawsoupletters [text]\n"
                            ret += "  鈥� Drawcookies [text]\n"
                            ret += "  鈥� Themes [query]\n"
                            ret += "  鈥� Stickers [query]\n"
                            ret += "  鈥� Goimage2 [text]\n"
                            ret += "  鈥� Instagram2 [text]\n"
                            ret += "  鈥� Devianart [text]\n"
                            ret += "  鈥� Food2 [text]\n"
                            ret += "  鈥� Calc [num]\n"
                            ret += "  鈥� Urban [query]\n"
                            ret += "  鈥� Randomname\n"
                            ...
                            ret += "  鈥� Creepypasta\n"
                            ret += "  鈥� Kaskus [query]\n"
                            ret += "  鈥� Arti-nama [nama]\n"
                            ret += "  鈥� Cosplay [name]\n"
                            ret += "  鈥� Viloid [name]\n"
                            ret += "  鈥� Zodiac [query]\n"
                            ret += "  鈥� Zalgo [name]\n"
                            ret += "  鈥� Webtoon\n"
                            ret += "  鈥� Youtubemp4 [URL]\n"
                            ret += "  鈥� Youtubemp3 [URL]\n"
                            ret += "  鈥� Youtubelink [text]\n"
                            ret += "  鈥� Fileytmp3 [URL]\n"
                            ret += "  鈥� Video [query]\n"
                            ret += "  鈥� Fileytmp4 [URL]\n"
                            ret += "  鈥� Google [text]\n"
                            ret += "  鈥� Wikipedia [text]\n"
                            ret += "  鈥� Topnews\n"
                            ret += "  鈥� Tvschedule\n"
                            ret += "  鈥� Dick [name]\n"
                            ret += "  鈥� Tits [name]\n"
                            ...
                            ret += "  鈥� Doujinnews\n"
                            ret += "  鈥� Quran\n"
                            ret += "  鈥� Githubprofile [username]\n"
                            ret += "  鈥� Githubfollowing [username]\n"
                            ret += "  鈥� Githubfollowers [username]\n"
                            ret += "  鈥� Githubrepo [username]\n"
                            ret += "  鈥� Githuzip [name|name]\n"
                            ret += "  鈥� Fmylife\n"
                            ret += "  鈥� Single\n"
                            ret += "  鈥� Motivate"
                            hello = "{}".format(str(ret))
                            ...
                            ret += "  鈥� Openqr\n"
                            ret += "  鈥� Closeqr\n"
                            ret += "  鈥� Inviteid [Idline]\n  鈥� Invite [@]\n"
                            ret += "  鈥� Countdown [num]\n"
                            ret += "  鈥� Countforward [num]\n"
                            ret += "  鈥� Grouplist\n"
                            ret += "  鈥� Creategroup [name]\n"
                            ret += "  鈥� Changegn [name]\n"
                            ret += "  鈥� Changedp group"
                            hello = "{}".format(str(ret))
                            cu = client.getProfileCoverURL(clientMID)
                            image = str(cu)                            
                            data = {
                            ...
                            ret = "Type: Mention\n\n"
                            ret += "  鈥� Mention [num|>|<|1-5]\n"
                            ret += "  鈥� Mentionname [A-z]\n"
                            ret += "  鈥� Mention [A-z]\n"
                            ret += "  鈥� Mention [2|@]\n"
                            ret += "  鈥� Mentionall\n"
                            ret += "  鈥� Check mention"
                            ...
                            ret = "Type: Friend\n\n"
                            ret += "  鈥� Friendlist\n"
                            ret += "  鈥� Friendinfo [num]\n"
                            ret += "  鈥� Clearfriend\n"
                            ret += "  鈥� Delete <pm>\n"
                            ret += "  鈥� Delfriend [@]\n"
                            ret += "  鈥� Del friend [num]\n"
                            ret += "  鈥� Addfriend [@]"
                            hello = "{}".format(str(ret))
                            cu = client.getProfileCoverURL(clientMID)
                            image = str(cu)                            
                            ...
                            ret = "Type: Protection\n\n"
                            ret += "  鈥� Prokick On/Off\n"
                            ret += "  鈥� Projoin On/Off\n"
                            ret += "  鈥� Proinv On/Off\n"
                            ret += "  鈥� Proqr On/Off\n"
                            ret += "  鈥� Protectall On/Off\n"
                            ret += "  鈥� Proteclist"
                            hello = "{}".format(str(ret))
                            cu = client.getProfileCoverURL(clientMID)
                            image = str(cu)                            
                            ...
                            ret = "Type: Wordban\n\n"
                            ret += "  鈥� Wordbanadd [text]\n"
                            ret += "  鈥� Wordbandel [text]\n"
                            ret += "  鈥� List wordban"
                            hello = "{}".format(str(ret))
                            cu = client.getProfileCoverURL(clientMID)
                            image = str(cu)                            
                            ...
                            ret += "  鈥� Quranlist\n"
                            ret += "Get Ayat Qur'an\n"
                            ret += "  鈥� Qur'an [numsurah]\n"
                            ret += "  鈥� Qur'an [numsurah] [1|<|>|-]"
                            hello = "{}".format(str(ret))
                            cu = client.getProfileCoverURL(clientMID)
                            image = str(cu)                            
                            data = {
                            ...
                            ret = "Type: Broadcast\n\n"
                            ret += "  鈥� Gbroadcast [text]\n"
                            ret += "  鈥� Fbroadcast [text]\n"
                            ret += "  鈥� Allbroadcast [text]\n"
                            ret += "  鈥� Groupcastvoice [text]\n"
                            ret += "  鈥� Friendcastvoice [text]"
                            hello = "{}".format(str(ret))
                            cu = client.getProfileCoverURL(clientMID)
                            image = str(cu)                            
                            data = {
                            ...
                            ret = "Drama :\n  鈥� Webtoon drama\n  鈥� Webtoon drama [num]\n"
                            ret += "Fantasi :\n  鈥� Webtoon fantasi\n  鈥� Webtoon fantasi [num]\n"
                            ret += "Comedy :\n  鈥� Webtoon comedy\n  鈥� Webtoon comedy [num]\n"
                            ret += "Slice Of Life :\n  鈥� Webtoon sol\n  鈥� Webtoon sol [num]\n"
                            ret += "Romance :\n  鈥� Webtoon romance\n  鈥� Webtoon romancethriller [num]\n"
                            ret += "Thriller :\n  鈥� Webtoon thriller\n  鈥� Webtoon thriller [num]\n"
                            ret += "Horror :\n  鈥� Webtoon horror\n  鈥� Webtoon horror [num]"
                            hello = "{}".format(str(ret))
                            cu = client.getProfileCoverURL(clientMID)
                            image = str(cu)                            
                            data = {
                            ...
                            ret ="Type: List\n\n  鈥� Mysticker\n  鈥� Add sticker 銆宯ame銆峔n  鈥� Del sticker 銆宯ame銆峔n  鈥� Changesticker 銆宯ame銆峔n  鈥� List sticker\n  鈥� Sendsticker 銆宯umb銆� 銆宯ame銆峔n"
                            ret +="  鈥� Add pict 銆宯ame銆峔n  鈥� Del pict 銆宯ame銆峔n  鈥� Changepict 銆宯ame銆峔n  鈥� List pict\n  鈥� Sendpict 銆宯umb銆� 銆宯ame銆峔n"
                            ret +="  鈥� Addaudio銆宯ame銆峔n  鈥� Delaudio銆宯ame銆峔n  鈥� Audiolist\n"
                            ret +="  鈥� Addtext銆宯ame~/ text 銆峔n  鈥� Deltext銆宯ame銆峔n  鈥� List text\n"
                            ret +="  鈥� Addtext2銆宯ame~/ text 銆峔n  鈥� Deltext2銆宯ame銆峔n  鈥� List text2"
                            hello = "{}".format(str(ret))
                            cu = client.getProfileCoverURL(clientMID)
                            image = str(cu)                            
                            data = {
                            ...
                            ret = "How to use ?\n"
                            ret += "Use command :\n  鈥� Creatememe *text*|*text*|*template*\n"
                            ret += "Example :\n"
                            ret += "  鈥� Creatememe khie|ganteng|buzz\n"
...
#==========================================
                            ...
                            client.generateReplyMessage(msg.id)
                            client.sendReplyMessage(msg.id, to, "銆� Mid 銆峔n "+str(sender))
                        elif cmd == "myprofile":
                            contact = client.getContact(clientMID)
                            cu = client.getProfileCoverURL(clientMID)
                            path = str(cu)
                            ...
                            client.sendMessage(to, "銆� Profile 銆峔nMid : "+str(sender)+"\nName : "+str(contact.displayName)+"\nStatus :\n"+str(contact.statusMessage))
                        elif cmd == "myname":
                            h = client.getContact(clientMID)
                            client.generateReplyMessage(msg.id)
                            client.sendReplyMessage(msg.id, to, "銆� Name 銆峔n"+str(h.displayName))
                        elif cmd == "mybio":
                            h = client.getContact(clientMID)
                            client.generateReplyMessage(msg.id)
                            client.sendReplyMessage(msg.id, to, "銆� Status 銆峔n"+str(h.statusMessage))                            
                        elif cmd == "mypicture":
                            h = client.getContact(clientMID)
                            ...
                            if h.videoProfile == None:
                            	return client.sendMessage(to, "銆� Video 銆峔nNone")
                            client.generateReplyMessage(msg.id)
                            ...
                            if msg.toType == 2:
                                group = client.getGroup(to)
                                client.sendMessage(to, "馃敟Bye bye "+str(group.name))
                                client.leaveGroup(to)
                        elif cmd == "timeleft":
                            timeNow = time.time()
                            runtime = timeNow - botStart
                            runtime = format_timespan(runtime)
                            client.sendMessage(to, "銆� Time to due 銆峔n"+str(runtime))
                        elif cmd == "rejectall" and sender == clientMID:
                            ginvited = client.getGroupIdsInvited()
                            if ginvited != [] and ginvited != None:
                                for gid in ginvited:
                                    client.rejectGroupInvitation(gid)
                                client.sendMessage(to, "Succes rejected 銆� {} 銆峣nvitetation".format(str(len(ginvited))))
                            else:
                            ...
                            if msg.toType == 2:
                                ...
                                if group.invitee is None or group.invitee == []:
                                    ...
                                    for inv in invitee:
                                        ...
                                        time.sleep(1)
                                    client.sendMessage(to, "Succes canceled 銆� {} 銆島ser".format(str(len(invitee))))
                        elif cmd == "nukeall":
                            if msg.toType == 2:
                                group = client.getGroup(to)
...
#                              \\ COMMAND PROFILE //
                            ...
                            num=1
                            msgs="銆� Favorite List 銆峔n"
                            for ids in kontak:
                                msgs+="\n%i. %s" % (num, ids.displayName)
                                num=(num+1)
                            msgs+="\n\n銆� Total Favoritelist : %i 銆�" % len(kontak)
                            msgs+= "\nUsage : FavoriteInfo銆� number 銆�"
                            hello = "{}".format(str(msgs))
                            data = {
                                "type": "text",
                                "text": "{}".format(str(msgs)),
...
#=====================================================================
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    client.sendGift(group,str(message),'sticker')
                                client.sendMessage(to, "銆� Remote Gift 銆峔n\nGroup : " + G.name)
                            except Exception as error:
                                client.sendMessage(to, str(error))
                            ...
                            try:
                                ...
                                try:
                                    client.sendMessage(to,"銆� Block Info 銆峔n\n" + "Name : " + friend.displayName + "\nStatus : " + friend.statusMessage + "\nMid : " + friend.mid)
                                    client.sendImageWithURL(to,image)
                                    client.sendImageWithURL(to,path)
                                ...
                                client.sendMessage(to, "銆� Result Error 銆峔n" + str(error))
                            ...
                            try:
                                ...
                                image = "http://dl.profile.line-cdn.net/" + friend.pictureStatus
                                try:
                                    client.sendMessage(to,"銆� Favorite Info銆峔n\n" + "Name : " + friend.displayName + "\nStatus : " + friend.statusMessage + "\nMid : " + friend.mid)
                                    client.sendImageWithURL(to,image)
                                    client.sendImageWithURL(to,path)
                                    client.sendContact(to, friend.mid)
                                except:
                            ...
                            if " " in key:
                                ...
                                hoho["savefile"] = True
                                client.sendMessage(to, "Send file to save to be銆� {} 銆�".format(str(key).lower()))
                                            ...
                                            if len(ret_) >= 10:
                                                ...
                                                ret_.append({
                                                    ...
                                                    "action": {
                                                        "type": "uri",
                                                        "label": "Send Image",
                                                        "uri": "line://app/1632242027-drK8Q1KZ?type=image&img={}".format(str(fn["link"]))
                                                        }
                                                    }
                                                )
                                    k = len(ret_)//10
                                            ...
                                            if len(ret_) >= 10:
                                                ...
                                                ret_.append({
                                                    ...
                                                    "action": {
                                                        "type": "uri",
                                                        "label": "Send Image",
                                                        "uri": "line://app/1632242027-drK8Q1KZ?type=image&img={}".format(str(fn["img"]))
                                                        }
                                                    }
                                                )
                                    k = len(ret_)//10
                        ...
                        if(cmd.startswith('youtube video ') or cmd.startswith('youtube audio ') or cmd.startswith('youtube info ')):
                            try:
                                ...
                                if cmd.startswith('youtube info '):
                                    ...
                                    t = ' 銆� Youtube Info 銆峔n喙�喔｀阜喙堗腑喔�: {}{}\n\n喔娻腑喔�: {}\n喙勦浮喙堗笂喔笟: {}\n喔堗赋喔權抚喔權竸喔權笖喔�: {}'.format(meta['title'],hjk,humanize.intcomma(meta['like_count']),humanize.intcomma(meta['dislike_count']),humanize.intcomma(meta['view_count']))
                                    kntlRm(to,t)
                                    s = meta['thumbnail']
                                    anunanu(to,s,wait)
                                            ...
                                            if len(ret_) >= 10:
                                                ...
                                                ret_.append({
                                                    ...
                                                    "action": {
                                                        "type": "uri",
                                                        "label": "Send Image",
                                                        "uri": "line://app/1632242027-drK8Q1KZ?type=image&img={}".format(str(food["url"]))
                                                        }
                                                    }
                                                )
                                    k = len(ret_)//10
                            ...
                            if 'MENTION' in msg.contentMetadata.keys()!= None:
                                ...
                                for ls in lists:
                                    ...
                                    try:
                                	    ...
                                	    rename = "銆� Display Rename 銆峔nERROR"
                                    client.sendMessage(to, "銆� Display Name 銆峔n" + contact.displayName + "\n\n銆� Display Rename 銆峔n{}".format(rename))
...
#                              \\ COMMAND Steal //
                            ...
                            if 'MENTION' in msg.contentMetadata.keys() != None:
                                ...
                                if ngroup == "":
                                    client.sendMessage(to, "銆� Locate 銆峔nNot found")
...
#=====================================================================
                            ...
                            if settings['setKey'] == False:key = ""
                            client.generateReplyMessage(msg.id)
                            client.sendReplyMessage(msg.id, to, "銆� Auto Join 銆峔nEvent Trigger:\n Autojoin: "+a+"\n Stage: "+b+"\n\nCommand:\n Autojoin\n 鈥� Usage: "+key+" autojoin on|off\n 鈥� Usage: "+key+" autojoin set 銆宯umb銆�")
                        elif cmd.startswith("autojoin set "):
                            msg.text = client.mycmd(msg.text,wait)
                            wait["Members"] = int(msg.text.split(" ")[2])
                            client.sendMessage(msg.to, " 銆� Autojoin 銆峔nType: Minim Members\nStatus: Success Set\nTo: {} Members".format(wait["Members"]))
                        elif cmd == "autojoin on":
                            if settings['autoJoin'] == True:
                                msgs=" 銆� Auto Join 銆峔nAuto Join already set to ENABLED鈾�"
                            else:
                                msgs=" 銆� Auto Join 銆峔nAuto Join has been set to ENABLED鈾�"
                                settings['autoJoin']=True
                            client.sendMessage(msg.to, msgs)
                        elif cmd == "autojoin off":
                            if settings['autoJoin'] == False:
                                msgs=" 銆� Auto Join 銆峔nAuto Join already set to DISABLED鈾�"
                            else:
                                msgs=" 銆� Auto Join 銆峔nAuto Join has been set to DISABLED鈾�"
...
#=====================================================================
                            ...
                            key = settings['keyCommand'].title()
                            if settings['setKey'] == False:key = ''
                            if settings["readerPesan"] is not None:ret = " 銆� Get Reader 銆峔nGetreader Message : " + str(settings["readerPesan"])
                            else:ret = " 銆� Get Reader 銆峔nGetreader Message: None"
                            ...
                            try:
                                ...
                                client.sendMessage(to," 銆� Get Reader 銆峔nChanged to : " + text_)
                            except:
                                client.sendMessage(to," 銆孏et Reader 銆峔nFailed to replace message")
...
#=====================================================================
                            ...
                            if settings['setKey'] == False:key = ""
                            client.generateReplyMessage(msg.id)
                            client.sendReplyMessage(msg.id, to, " 銆� Auto Read 銆峔nEvent Trigger:\n1 on Personal: "+a+"\n2 on Group: "+b+"\n\nCommand:\n Autoread\n  Usage: "+key+" autoread on 1|2 or off 1|2")
                        elif cmd == "autoread on 1":
                            if settings["autoRead"] == True:
                                msgs=" 銆� Auto Read 銆峔nAutoread in Private message set to ENABLED鈾猏nMessage will not Affected.."
                            else:
                                msgs=" 銆� Auto Read 銆峔nAutoread in Private message already set to ENABLED鈾猏nMessage will not Affected.."
                                settings["autoRead"]=True
                            client.sendMessage(msg.to, msgs)
                        elif cmd == "autoread off 1":
                            if settings["autoRead"] == False:
                                msgs=" 銆� Auto Read 銆峔nAutoread in Private message set to DISABLED鈾猏nMessage will Affected.."
                            else:
                                msgs=" 銆� Auto Read 銆峔nAutoread in Private message already set to DISABLED鈾猏nMessage will Affected.."
                                settings["autoRead"]=False
                            client.sendMessage(msg.to, msgs)
                        elif cmd == "autoread on 2":
                            if settings["autoRead1"] == True:
                                msgs =" 銆� Auto Read 銆峔nAutoread in Group set to ENABLED鈾猏nMessage will not Affected.."
                            else:
                                msgs=" 銆� Auto Read 銆峔nAutoread in Group already set to ENABLED鈾猏nMessage will not Affected.."
                                settings["autoRead1"]=True
                            client.sendMessage(msg.to, msgs)
                        elif cmd == "autoread off 2":
                            if settings["autoRead1"] == False:
                                msgs=" 銆� Auto Read 銆峔nAutoread in Group set to DISABLED鈾猏nMessage will Affected.."
                            else:
                                msgs=" 銆� Auto Read 銆峔nAutoread in Group set to DISABLED鈾猏nMessage will Affected.."
                            ...
                            if settings["replyPesan"] is not None:
                                client.sendMessage(to,"銆� Sleep Mode 銆峔nSet Sleep Mode : " + str(settings["replyPesan"]))
                                msgSticker = settings["messageSticker"]["listSticker"]["replySticker"]
                                if msgSticker != None:
                                    sid = msgSticker["STKID"]
                                    spkg = msgSticker["STKPKGID"]
                            ...
                            try:
                                ...
                                client.sendMessage(to,"銆� Sleep Mode 銆峔nChanged to : " + text_)
                            except:
                                client.sendMessage(to,"銆� Sleep Mode 銆峔nFailed to replace message")
                        elif cmd == "temp off":
                            if norak["detectTemplate"] == False:
                                msgs=" 銆� Detect Template 銆峔nDetect Template already DISABLED鈾猏nNote: Detect Template message is not affected鈾�"
                            else:
                                msgs=" 銆� Detect Template 銆峔nDetect Template set to DISABLED鈾猏nNote: Detect Template message is not affected鈾�"
                                norak['detectTemplate']=False
                            client.sendMessage(to, msgs)
                        elif cmd == "temp on":
                            if norak["detectTemplate"] == True:
                                msgs=" 銆� Detect Template 銆峔nDetect Template Enable鈾猏nNote: Detect Template message is not affected鈾�"
                            else:
                                msgs=" 銆� Detect Template 銆峔nDetect Template set to Enable鈾猏nNote: Detect Template message is not affected鈾�"
                                norak["detectTemplate"]=True
                            client.sendMessage(to, msgs)
                        elif cmd == "antitag on":
                            if settings["notag"] == True:
                                msgs=" 銆� Anti Tag 銆峔nAnti Tag already Enable鈾�"
                            else:
                                msgs=" 銆� Anti Tag 銆峔nAnti Tag set to Enable鈾�"
                                settings["notag"] = True
                            client.sendMessage(to, msgs)
                        elif cmd == "antitag off":
                            if settings["notag"] == False:
                                msgs=" 銆� Anti Tag 銆峔nAnti Tag already DISABLED鈾�"
                            else:
                                msgs=" 銆� Anti Tag 銆峔nAnti Tag set to DISABLED鈾�"
                                settings["notag"] = False
                            client.sendMessage(to, msgs)
                        elif cmd == "sleepmode on":
                            if settings["autoReply"] == True:
                                msgs=" 銆� Sleep Mode 銆峔nSleep Mode already Enable鈾�"
                            else:
                                msgs=" 銆� Sleep Mode 銆峔nSleep Mode set to Enable鈾�"
                                settings["autoReply"] = True
                            client.sendMessage(to, msgs)
                        elif cmd == "sleepmode off":
                            if settings["autoReply"] == False:
                                msgs=" 銆� Sleep Mode 銆峔nSleep Mode already DISABLED鈾�"
                            else:
                                msgs=" 銆� Sleep Mode 銆峔nSleep Mode set to DISABLED鈾�"
                                settings["autoReply"] = False
                            client.sendMessage(to, msgs)
                        elif cmd == "resendchat on":
                            if settings["unsendMessage"] == True:
                                msgs=" 銆� Resend Chat 銆峔nResend Chat already Enable鈾�"
                            else:
                                msgs=" 銆� Resend Chat 銆峔nResend Chat set to Enable鈾�"
                                settings["unsendMessage"] = True
                            client.sendMessage(to, msgs)
                        elif cmd == "resendchat off":
                            if settings["unsendMessage"] == False:
                                msgs=" 銆� Resend Chat 銆峔nResend Chat already DISABLED鈾�"
                            else:
                                msgs=" 銆� Resend Chat 銆峔nResend Chat set to DISABLED鈾�"
                                settings["unsendMessage"] = False
                            client.sendMessage(to, msgs)

                        elif cmd == "stealtag on":
                            if temptag["stealtag"] == True:
                                msgs=" 銆� Steal Tag 銆峔nSteal Tag already Enable鈾�"
                            else:
                                msgs=" 銆� Steal Tag 銆峔nSteal Tag set to Enable鈾�"
                                temptag["stealtag"] = True
                            client.sendMessage(to, msgs)
                        elif cmd == "stealtag off":
                            if temptag["stealtag"] == False:
                                msgs=" 銆� Steal Tag 銆峔nSteal Tag already DISABLED鈾�"
                            else:
                                msgs=" 銆� Steal Tag 銆峔nSteal Tag set to DISABLED鈾�"
...
#=====================================================================
                            ...
                            if settings["autoAdd"] == True:
                                if settings["addPesan"] == '':
                                    msgs=" 銆� Auto Add 銆峔nAdd Back: True鈾猏nAdd Sticker: "+c+"\nAdd Message: False鈾猏n\n\n"
                                else:
                                    msgs=" 銆� Auto Add 銆峔nAdd Back: True鈾猏nAdd Sticker: "+c+"\nAdd Message: True鈾�"
                                    msgs+="\n" + settings["addPesan"] + "\n\n"
                            else:
                                if settings["addPesan"] == '':
                                    msgs=" 銆� Auto Add 銆峔nAdd Back: False鈾猏nAdd Sticker: "+c+"\nAdd Message: False鈾猏n\n\n"
                                else:
                                    msgs=" 銆� Auto Add 銆峔nAdd Back: False鈾猏nAdd Sticker: "+c+"\nAdd Message: True鈾�"
                                    msgs+="\n" + settings["addPesan"] + "\n"
                            client.generateReplyMessage(msg.id)
                            ...
                            if settings["autoAdd"] == False:
                                msgs=" 銆� Auto Add 銆峔nAuto Add already DISABLED鈾猏nNote: Auto add message is not affected鈾�"
                            else:
                                msgs=" 銆� Auto Add 銆峔nAuto Add set to DISABLED鈾猏nNote: Auto add message is not affected鈾�"
                                settings['autoAdd']=False
                            client.sendMessage(to, msgs)
                        elif cmd == "autoadd on":
                            if settings["autoAdd"] == True:
                                msgs=" 銆� Auto Add 銆峔nAuto Add already Enable鈾猏nNote: Auto add message is not affected鈾�"
                            else:
                                msgs=" 銆� Auto Add 銆峔nAuto Add set to Enable鈾猏nNote: Auto add message is not affected鈾�"
                            ...
                            if len(msg.text.split("\n")) >= 2:
                                ...
                                client.sendMessage(msg.to," 銆� Auto Add 銆峔nAuto add message has been set to:\n" + settings["addPesan"])
                        elif cmd == "add autoaddsticker" and sender == clientMID:
                            settings["messageSticker"]["addStatus"] = True
                            settings["messageSticker"]["addName"] = "addSticker"
                            client.sendMessage(to, " 銆� Auto Add 銆峔nType: Auto add\n 鈥� Detail: Add autoadd sticker\n 鈥� Status: Send sticker")
                        elif cmd == "del autoaddsticker" and sender == clientMID:
                            settings["messageSticker"]["listSticker"]["addSticker"]["status"] = False
                            client.sendMessage(to, " 銆� Auto Add 銆峔nType: Auto add\n 鈥� Detail: Del autoadd sticket\n 鈥� Status: Succes")
                            ...
                            client.sendMessage(msg.to, " 銆� Sticker 銆峔nSend the sticker")
                        elif cmd == "del autoresponsticker":
                          if msg.to in wait['GROUP']['AR']['S']:
                              wait['GROUP']['AR']['S'] = {}
                              client.sendMessage(msg.to, " 銆� Sticker 銆峔nSucces delete sticker")
                        elif cmd == "autorespon on":
                            if msg.to in wait["GROUP"]['AR']['AP']:
                                msgs=" 銆� Auto Respon 銆峔nAuto Respon already ENABLED鈾�"
                            else:
                                msgs=" 銆� Auto Respon 銆峔nAuto Respon set to ENABLED鈾�"
                                wait["GROUP"]['AR']['AP'].append(msg.to)
                            client.sendMessage(to, msgs)
                        elif cmd == "autorespon off":
                            if msg.to not in wait["GROUP"]['AR']['AP']:
                                msgs=" 銆� Auto Respon 銆峔nAuto Respon already DISABLED鈾�"
                            else:
                                msgs=" 銆� Auto Respon 銆峔nAuto Respon set to DISABLED鈾�"
                            ...
                            if settings['setKey'] == False:key = ''
                            if msg.to in wait["GROUP"]['AR']['AP']:
                                msgs=" 銆� Auto Respon 銆峔nAuto Respon: ON鈾�"
                                if msg.to in wait["GROUP"]['AR']['S']:
                                ...
                                msgs=" 銆� Auto Respon 銆峔nAuto Respon: OFF"
                                if msg.to in wait["GROUP"]['AR']['S']:
                                    a = client.shop.getProduct(packageID=int(wait["GROUP"]['AR']['S'][msg.to]['Sticker']['STKPKGID']), language='ID', country='ID')
                                    msgs+="\nSticker: " + a.title
                                else:msgs+=''
                            ...
                            if len(msg.text.split("\n")) >= 2:
                                ...
                                client.sendMessage(msg.to," 銆� Auto Respon 銆峔nAuto Respon message has been set to:\n" + wait["GROUP"]['AR']['P'][msg.to])
                        elif cmd == "leavemsg":
                            key = settings['keyCommand'].title()
                            if settings['setKey'] == False:key = ''
                            if msg.to in wait["GROUP"]['LM']['AP']:
                                msgs=" 銆� Leave Message 銆峔nLeave Message: ON鈾�"
                                ...
                                msgs=" 銆� Leave Message 銆峔nLeave Message: OFF"
                                if msg.to in wait["GROUP"]['LM']['S']:
                                    a = client.shop.getProduct(packageID=int(wait["GROUP"]['LM']['S'][msg.to]['Sticker']['STKPKGID']), language='ID', country='ID')
                                    msgs+="\nSticker: " + a.title
                                else:msgs+=''
                            ...
                            if msg.to in wait["GROUP"]['LM']['AP']:
                                msgs=" 銆� Leave Message 銆峔nLeave Message already ENABLED鈾�"
                            else:
                                msgs=" 銆� Leave Message 銆峔nLeave Message set to ENABLED鈾�"
                                wait["GROUP"]['LM']['AP'].append(msg.to)
                            client.sendMessage(to,msgs)
                        elif cmd == 'leave off':
                            if msg.to not in wait["GROUP"]['LM']['AP']:
                                msgs=" 銆� Leave Message 銆峔nLeave Message already DISABLED鈾�"
                            else:
                                msgs=" 銆� Leave Message 銆峔nLeave Message set to DISABLED鈾�"
                            ...
                            if msg.to in wait['GROUP']['LM']['S']:
                                wait['GROUP']['LM']['S'] = {}
                                client.sendMessage(to, " 銆� Sticker 銆峔nSucces delete sticker")
                        elif cmd.startswith('leave msg set'):
                            msg.text = client.mycmd(msg.text,wait)
                            if len(msg.text.split("\n")) >= 2:
                                wait["GROUP"]['LM']['P'][msg.to] = msg.text.replace(msg.text.split("\n")[0]+"\n","")
                                client.sendMessage(msg.to," 銆� Leave Message 銆峔nLeave Message has been set to:\n" + wait["GROUP"]['LM']['P'][msg.to])
                        elif cmd == "welcome on":
                            if msg.to in wait["GROUP"]['WM']['AP']:
                                msgs=" 銆� Welcome Message 銆峔nWelcome Message already ENABLED鈾�"
                            else:
                                msgs=" 銆� Welcome Message 銆峔nWelcome Message set to ENABLED鈾�"
                                wait["GROUP"]['WM']['AP'].append(msg.to)
                            client.sendMessage(to,msgs)
                        elif cmd == "welcome off":
                            if msg.to not in wait["GROUP"]['WM']['AP']:
                                msgs=" 銆� Welcome Message 銆峔nWelcome Message already DISABLED鈾�"
                            else:
                                msgs=" 銆� Welcome Message 銆峔nWelcome Message set to DISABLED鈾�"
                            ...
                            if len(msg.text.split("\n")) >= 2:
                                ...
                                client.sendMessage(msg.to," 銆� Welcome Message 銆峔nWelcome Message has been set to:\n" + wait["GROUP"]['WM']['P'][msg.to])
                        elif cmd == 'welcomemsg':
                            key = settings['keyCommand'].title()
                            if settings['setKey'] == False:key = ''
                            if msg.to in wait["GROUP"]['WM']['AP']:
                                msgs=" 銆� Welcome Message 銆峔nWelcome Message: ON鈾�"
                                ...
                                msgs=" 銆� Welcome Message 銆峔nWelcome Message: OFF"
                                if msg.to in wait["GROUP"]['WM']['S']:
                                    a = client.shop.getProduct(packageID=int(wait["GROUP"]['WM']['S'][msg.to]['Sticker']['STKPKGID']), language='ID', country='ID')
                                    msgs+="\nSticker: " + a.title
                                else:msgs+=''
...
#=====================================================================
  ...
  #                              client.generateReplyMessage(msg.id)
  #                              client.sendReplyMessage(msg.id, to," 銆� Unsend 銆峔nUnsend {} message".format((n-t)))
                            if len(msg.text.split(' ')) >= 3:h = [client.unsendMessage(client.sendMessage(to,client.adityasplittext(msg.text,'s')).id) for b in a]
#=====================================================================
#=====================================================================
                        elif cmd == "friends":
                            client.sendMessage(to, " 銆� Friend 銆峔nType: List contact\n 鈥� Usage: Friendlist\n 鈥� Usage: Friend info 銆宯umb銆峔n\nType: Delete friend\n 鈥� Usage: Clearfriend\n 鈥� Usage: Delete\n 鈥� Usage: Delfriend 銆� @ 銆峔n 鈥� Usage: Del friend 銆宯umb銆�")
                            ...
                            client.generateReplyMessage(msg.id)
                            client.sendReplyMessage(msg.id, to,"Type: Friendlist\n 鈥� Detail: Clear Contact\n 鈥� Before: %s Friendlist\n 鈥� After: %s Friendlist\n 鈥� Total removed: %s Friendlist\n 鈥� Status: Succes.."%(n,t,(n-t)))
                        elif cmd == "delete":
                            n = len(client.getAllContactIds())
                            try:
                            ...
                            t = len(client.getAllContactIds())
                            client.generateReplyMessage(msg.id)
                            client.sendReplyMessage(msg.id, to, "Type: Friendlist\n 鈥� Detail: Delete friend\n 鈥� Status: Succes..\n 鈥� Before: %s Friendlist\n 鈥� After: %s Friendlist"%(n,t))
...
#=====================================================================
                            ...
                            k = len(nama)//20
                            for a in range(k+1):
                                if a == 0:client.mentionmention(to=msg.to,wait=wait,text='',dataMid=nama[:20],pl=0,ps='鈺�� Mention 銆嶁攢',pg='MENTIONALLUNSED',pt=nama)
                                else:client.mentionmention(to=msg.to,wait=wait,text='',dataMid=nama[a*20 : (a+1)*20],pl=a*20,ps='鈹溿�� Mention 銆嶁攢',pg='MENTIONALLUNSED',pt=nama)
                        elif cmd == "mentionall":
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    client.leaveGroup(G.id)
                                client.sendMessage(to, "銆孡eave 銆峔n\nGroup : " + G.name)
                            except Exception as error:
                                client.sendMessage(to, str(error))
#=====================================================================
#=====================================================================
                            ...
                            if to in wait["lurkp"]:
                                if wait["lurkp"][to] == {}:
                                    b='\n鈺癓urk People: None'
                                    h="鈺�� Lurk 銆嶁攢"+a+"\n鈹�    | Command |  \n鈹侺urtk Point\n鈹�  Usage: lurk on\n鈹�  Usage: lurk auto on\n鈹侺urk Del\n鈹�  Usage: lurk off\n鈹�  Usage: lurk auto off\n鈹侺urk Cek\n鈹�  Usage: lurk result"
                                    client.sendMessage(to,h+b)
                                else:
                                    h= "鈺�� Lurk 銆嶁攢"+a+"\n鈹�    | Command |  \n鈹侺urk Point\n鈹�  Usage: lurk on\n鈹�  Usage: lurk auto on\n鈹侺urk Del\n鈹�  Usage: lurk off\n鈹�  Usage: lurk auto off\n鈹侺urk Cek\n鈹�  Usage: lurk result\n鈹侺urk People: {}".format(len(wait["lurkp"][to]))
                                    no=0
                                    hh = []
                            ...
                            if to in wait['readPoint']:
                                client.sendMessage(to, " 銆� Lurk 銆峔nLurk already set鈾�")
                            else:
                                try:
                                ...
                                client.sendMessage(to, " 銆� Lurk 銆峔nLurk point set鈾�")
                        elif cmd == "lurk off":
                            if msg.to not in wait['readPoint']:
                                client.sendMessage(to, " 銆� Lurk 銆峔nLurk already off鈾�")
                            else:
                                try:
                                   del wait['readPoint'][to];wait['setTime'][to] = {};wait['ROM1'][to] = {}
                                except:
                                   pass
                                client.sendMessage(to, " 銆� Lurk 銆峔nLurk point off鈾�")
                        elif cmd == "lurk result":
                            if to in wait['readPoint']:
                                try:
                                    anulurk(to,wait)
                                    wait['setTime'][to]  = {}
                                except:client.sendMessage(to,'鈺�� Lurkers 銆嶁攢\n鈺� None')
                            else:client.sendMessage(to, " 銆� Lurk 銆峔nLurk point not on鈾�")
                        elif cmd == "lurk auto on":
                            if to in wait['readPoint']:
                                if wait['lurkauto'] == True:client.sendMessage(to, " 銆� Lurk 銆峔nLurk already set鈾�")
                            else:
                                try:
                                    del wait['readPoint'][to];del wait['setTime'][to]
                                except:
                            ...
                            if wait['lurkauto'] == False:
                                client.sendMessage(to, " 銆� Lurk 銆峔nLurk auto already off鈾�")
                            else:
                                wait['lurkauto'] = False
                                client.sendMessage(to, " 銆� Lurk 銆峔nLurk auto point off鈾�")
                        elif cmd.startswith("lurk on "):
                            if 'MENTION' in msg.contentMetadata.keys()!=None:
                                key = eval(msg.contentMetadata["MENTION"])
                                targets = key["MENTIONEES"][0]["M"]
                                if targets in wait['readPoints']:
                                    client.sendMention(to, " 銆� Lurk 銆峔nLurk in @! already active",'',[targets])
                                else:
                                    try:
                                    ...
                                    client.sendMention(to, " 銆� Lurk 銆峔nLurk in @! set to active",'',[targets])
                        elif cmd.startswith("lurk off "):
                            if 'MENTION' in msg.contentMetadata.keys()!=None:
                                key = eval(msg.contentMetadata["MENTION"])
                                targets = key["MENTIONEES"][0]["M"]
                                if targets not in wait['readPoints']:
                                    client.sendMention(to, " 銆� Lurk 銆峔nLurk in @! already mute",'',[targets])
                                else:
                                    try:
                                        del wait['readPoints'][targets];del wait['lurkp'][to];del wait["lurkt"][to]
                                    except:
                                        pass
                                    client.sendMention(to, " 銆� Lurk 銆峔nLurk in @! set to mute",'',[targets])
                            ...
                            if 'MENTION' in msg.contentMetadata.keys()!=None:
                                ...
                                if targets in wait['readPoints']:
                                    ...
                                    k = len(chiya)//100
                                    for a in range(k+1):
                                        if a == 0:client.mentionmention(to=to,wait=wait,text='',dataMid=chiya[:100],pl=0,ps='鈺�� Lurkers 銆嶁攢',pg='SIDERMES',pt=chiya)
                                        else:client.mentionmention(to=to,wait=wait,text='',dataMid=chiya[a*100 : (a+1)*100],pl=a*100,ps='鈹溿�� Lurkers 銆嶁攢',pg='SIDERMES',pt=chiya)
                                    wait['lurkt'][to][targets]  = {};wait['lurkp'][to][targets] = {}
                                else:client.sendMention(to, " 銆� Lurk 銆峔nLurk in @! not active",'',[targets])
#=====================================================================
#=====================================================================
                            ...
                            if name not in images:
                                ...
                                client.sendMessage(to, "Type: Picture\n 鈥� Detail: Add picture\n 鈥� Statud: Send pict...")
                            else:
                                client.sendMessage(to, "Type: Picture\n 鈥� Detail: Add picture\n 鈥� Status: Failed, Picture already in list...")
                            ...
                            if name in images:
                                ...
                                f = codecs.open('image.json','w','utf-8')
                                json.dump(images, f, sort_keys=True, indent=4, ensure_ascii=False)
                                client.sendMessage(to, "Type: Picture\n 鈥� Detail: Delete list picture\n 鈥� Status: Succes delete Picture {}".format(str(name.lower())))
                            else:
                                client.sendMessage(to, "Type: Picture\n 鈥� Detail: Delete list picture\n 鈥� Status: Failed, Picture not in list...")
                            ...
                            if name in images:
                                ...
                                f = codecs.open('image.json','w','utf-8')
                                json.dump(images, f, sort_keys=True, indent=4, ensure_ascii=False)
                                client.sendMessage(to, "Type: Picture\n 鈥� Detail: Change picture\n 鈥� Status: Send pict...")
                            else:
                                client.sendMessage(to, "Type: Picture\n 鈥� Detail: Change picture\n 鈥� Status: Failed, Picture not in list...")
                        elif cmd == "list pict":
                            load()
                            no = 0
                            ret_ = " 銆� List Picture 銆�"
                            for image in images:
                                no += 1
                            ...
                            if imagename in images:
                                ...
                                client.sendMessage(to, "Type: Picture\n 鈥� Detail: Send picture\n 鈥� Status: Failed, Picture name not in list...")
                                return
                            for x in range(jml):
...
#=====================================================================
                            ...
                            if name not in stickers:
                                ...
                                client.sendMessage(to, "Type: Stickers\n 鈥� Detail: Add sticker\n 鈥� Status: Send sticker..")
                            else:
                                client.sendMessage(to, "Type: Stickers\n 鈥� Detail: Add sticker\n 鈥� Status: Failed, Sticker name already in list..")
                            ...
                            if name in stickers:
                                ...
                                f = codecs.open('sticker.json','w','utf-8')
                                json.dump(stickers, f, sort_keys=True, indent=4, ensure_ascii=False)
                                client.sendMessage(to, "Type: Sticker\n 鈥� Detail: Delete sticker\n 鈥� Status: Succes delete Sticker {}".format(str(name.lower())))
                            else:
                                client.sendMessage(to, "Type: Sticker\n 鈥� Detail: Delete sticker\n 鈥� Status: Failed, Sticker name not in list")
                            ...
                            if name in stickers:
                                ...
                                f = codecs.open('sticker.json','w','utf-8')
                                json.dump(stickers, f, sort_keys=True, indent=4, ensure_ascii=False)
                                client.sendMessage(to, "Type: Stickers\n 鈥� Detail: Change sticker\n 鈥� Status: Send sticker..")
                            else:
                                client.sendMessage(to, "Type: Stickers\n 鈥� Detail: Change sticker\n 鈥� Status: Failed, Sticker not in list..")
                        elif cmd == "list sticker2":
                            load()
                            ret_ = "銆� Sticker List 銆峔n"
                            for sticker in stickers2:
                                ret_ += "\n" + sticker.title()
                            ...
                            with requests.session() as web:
                                ...
                                if len(cond) == 1:
                                    ...
                                    ret_ += "\n\nSearchLyric {}|銆宯umber銆�".format(str(query))
...
#=====================================================================
                            ...
                            if a == []:
                                client.sendMention(to, 'Sorry @!In {} nothink get a Announcements'.format(self.getGroup(to).name),' 銆� Announcements 銆峔n', [self.getProfile().mid])
                                return
                            no = 0
                            c = ' 銆� Announcements 銆�'
                            h = []
                            ...
                            a = client.getChatRoomAnnouncements(msg.to)
                            if a == []:
                                client.sendMention(to, 'Sorry @!In {} nothink get a Announcements'.format(client.getGroup(to).name),' 銆� Announcements 銆峔n', [client.getProfile().mid])
                                return
                            c = ' 銆� Announcements 銆�'
                            no = 0
                            h = []
                            ...
                            for b in a:
                                ...
                                if no == 1:c+= '\n{}. 銆� {} 銆峔n{}'.format(no,humanize.naturaltime(datetime.fromtimestamp(b.createdTime/1000)),sdg)
                                else:c+= '\n\n{}. 銆� {} 銆峔n{}'.format(no,humanize.naturaltime(datetime.fromtimestamp(b.createdTime/1000)),sdg)
                            ...
                            client.sendMessage(to, str(hasil))        
                                ...
                                if a["result"] != []:                                
                                    ...
                                    he = []
                                    for op in a['result']:
                                        ret_.append({"thumbnailImageUrl":"https://t.nhentai.net/galleries/{}/cover.jpg".format(op["media_id"]),"imageSize": "contain","imageAspectRatio": "square","title": '{}'.format(str(op["title"]["english"][:40])),"text": '{}'.format(str(op["title"]["english"][:40])),"actions": [{"type": "uri","label": "喔佮笖喙�喔炧阜喙堗腑喔｀副喔氞笂喔�","uri": 'https://nhentai.net/g/' + str(op["id"])}]})
                        		...
                        		"messages": [
                        			{
                        				...
                        				"template": {
                        					...
                        					"columns": [
                                                {
                                                	"title": "喔曕复喔斷笗喙堗腑喔溹腹喙夃釜喔｀箟喔侧竾",
                                                	"text": "AddMe",
                                                	"actions": [
                                                		{
                                                			"type": "uri",
                                                			"label": "ADD ME",
                                                			"uri": "line://ti/p/~jamerockzaza"
                                                		}
                                                	]
                                                },
                                                {
                                                	...
                                                	"actions": [
                                                		{
                                                			"type": "uri",
                                                			"label": "喔佮笖喔复",
                                                			"uri": "line://app/1487085176-lP805wzy?text=喔｀副喔佮笝喔班笗喔编抚喙�喔竾"                                                            
                                                		}                                                		
                                                	]
                                                }
                        					]
...
#####
                            ...
                            if bl["blacklist"] == []:
                                client.sendMessage(to,"銆� BlackList 銆�")
                            else:
                                num = 1
                                mc = "銆� BlackList 銆峔n"
                            ...
                            if 'MENTION' in msg.contentMetadata.keys()!=None:
                                ...
                                h = "鈺�� Find Contact 銆嶁攢"
                                no = 0
                                for group in c:
                                    no += 1
                                    h+= "\n鈹倇}. {} | {}".format(no, group.split(',.s/')[0], group.split(',.s/')[1])
                                client.sendMessage(msg.to,h+"\n鈺扳攢銆� {} Groups I Found it 銆�".format(len(c)))
                            ...
                            zxc = ""
                            zx2 = []
                            xpesan = '銆� Invited from Id 銆峔nName '
                            ...
                            groups = client.getGroupIdsJoined()
                            for group in groups:
                                client.sendMessage(group, "銆� Group Broadcast 銆峔n\n{}".format(str(txt)))
                                time.sleep(1)
                            ...
                            friends = client.getAllContactIds()
                            for friend in friends:
                                client.sendMessage(friend, "銆� Friend Broadcast 銆峔n\n{}".format(str(txt)))
                                time.sleep(1)
                            client.sendMessage(to, "Succes friend cast to {} friend ".format(str(len(friends))))
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    G.preventedJoinByTicket = True
                                    client.updateGroup(G)
                                client.sendMessage(to, "銆� Close Qr 銆峔n\nGroup : " + G.name)
                            except Exception as error:
                                client.sendMessage(to, str(error))
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    client.updateGroup(G)
                                    gurl = "https://line.me/R/ti/g/{}".format(str(client.reissueGroupTicket(G.id)))
                                client.sendMessage(to, "銆� Open Qr 銆峔n\nGroup : " + G.name + "\nLink: " + gurl)
                            except Exception as error:
                                client.sendMessage(to, str(error))
                            ...
                            if len(string) <= 10000000000:
                                ...
                                client.updateProfile(profile)
                                client.sendMessage(to, "銆� Update Name 銆峔nStatus : Success\nFrom : "+str(pname)+"\nTo :"+str(string))
                            ...
                            if len(string) <= 10000000000:
                                ...
                                client.updateProfile(profile)
                                client.sendMessage(to, "銆� Update Status 銆峔nStatus : Success\nFrom : "+str(pname)+"\nTo :"+str(string))
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    client.sendContact(group, "uc7d319b7d2d38c35ef2b808e3a2aeed9',")
                                client.sendMessage(to, "銆� Crash 銆峔n\nGroup : " + G.name)
                            except Exception as error:
                                client.sendMessage(to, str(error))
...
#=========================================================
                            ...
                            try:
                                ...
                                try:
                                    if int(msg.text.split(' ')[2]) > len(a):
                                        client.sendMessage(msg.to,' 銆� Webtoon 銆峔nDaftar Webtoon {} urutan ke {} tidak ditemukan'.format(drama.title(),msg.text.split(' ')[2]))
                                    ...
                                    A = ' 銆� Webtoon 銆峔n    | {} |'.format(a[int(msg.text.split(' ')[2])-1].find_all('p')[0].text)
                                    for c in range(0,10):
                                        if c+1 == 1:AA = '\n'
                                        else:AA = '\n\n'
                                    ...
                                    A = ' 銆� Webtoon 銆峔n    | {} |'.format(drama.replace('sol','slice of life').title())
                                    no=0
                                    for b in a:
                                        no+=1
                                        if no == 1:AA = '\n'
                            ...
                            if data["data"] != []:
                                no = 0
                                ret_ = "鈺攢鈹�銆� Al-Qur'an 銆�"
                                for music in data["data"]:
                                    no += 1
                                    if no == len(data['data']):ret_ += "\n鈺皗}. {}".format(no,music['englishName'])
                                    else:ret_ += "\n鈹倇}. {}".format(no,music['englishName'])
                                client.generateReplyMessage(msg.id)
                                client.sendReplyMessage(msg.id, to,ret_)
                            ...
                            if len(msg.text.split(' ')) == 2:
                                try:
                                    no = 0
                                    ret_ = " 銆� Al-Qur'an 銆峔nSurah: {}".format(data['data']['englishName'])
                                ...
                                except:client.sendMessage(msg.to," 銆� Al-Qur'an 銆峔nI can't found surah number {}".format(client.adityasplittext(msg.text)))
                            if len(msg.text.split(' ')) == 3:
                                try:
                                    nama = data["data"]["ayahs"]
                                    selection = client.MySplit(client.adityasplittext(msg.text.lower(),'s'),range(1,len(nama)+1))
                                    k = len(nama)//100
                                    text = " 銆� Al-Qur'an 銆峔nSurah: {}".format(data['data']['englishName'])
                                    no = 0
                                    ...
                                    client.sendMessage(msg.to," 銆� Al-Qur'an 銆峔nI can't found surah number {}".format(client.adityasplittext(msg.text)))
                            ...
                            if spl[0] == "":
                                if spl[1] != "":
                                    try:
                                        if msg.toType != 0:
                                            client.sendMessage(msg.to,"喔佮赋喔ム副喔囙福喔编笟喔傕箟喔浮喔灌弗 喔佮福喔膏笓喔侧福喔釜喔编竵喔勦福喔灌箞..")
                                        else:
                                            client.sendMessage(msg.from_,"喔佮赋喔ム副喔囙福喔编笟喔傕箟喔浮喔灌弗 喔佮福喔膏笓喔侧福喔釜喔编竵喔勦福喔灌箞..")
                                        resp = BeautifulSoup(requests.get("https://www.google.co.th/search",params={"q":spl[1],"gl":"th"}).content,"html.parser")
                                        text = "喔溹弗喔佮覆喔｀竸喙夃笝喔覆喔堗覆喔� Google:\n\n"
                            ...
                            if profile != [] and "message" not in profile:
                                ...
                                client.sendMessage(to, "鈺攢銆� Github 銆峔n鈺� Username not found")
                            ...
                            if len(cond) == 1:
                                if data != [] and "message" not in data:
                                    ret_ = "銆� Github Followers 銆峔n"
                                    for followers in data:
                                        no += 1
                                        ret_ += "\n" + str(no) + ". " + str(followers["login"])
                                    ret_ += "\n\n銆� Total {} User 銆�".format(str(len(data)))
                                    ret_ += "\n\nUsage : GithubFollowers {}|銆宯umber銆�".format(text)
                                    client.sendMessage(to,ret_)
                                ...
                                if data != [] and "message" not in data:
                                    if int(cond[1]) <= len(data):
                                        ...
                                        ret_ += "\n\n銆� https://github.com/{} 銆�".format(followers["login"])
                            ...
                            if len(cond) == 1:
                                if data != [] and "message" not in data:
                                    ret_ = "銆� Github RepoSitory 銆峔n"
                                    for repo in data:
                                        no += 1
                                        ret_ += "\n" + str(no) + ". " + str(repo["name"])
                                    ret_ += "\n\n銆� Total {} User 銆�".format(str(len(data)))
                                    ret_ += "\n\nUsage : GithubRepo {}|銆宯umber銆�".format(text)
                                    client.sendMessage(to,ret_)
                                elif "message" in data:
                                    client.sendMessage(to,"User not found")
                                elif data == []:
                                ...
                                if data != [] and "message" not in data:
                                    if int(cond[1]) <= len(data):
                                        ...
                                        ret_ += "\n\n銆� https://github.com/{} 銆�".format(str(repo["owner"]["login"]))
                            ...
                            if len(cond) == 1:
                                if data != [] and "message" not in data:
                                    ret_ = "銆� Github Following 銆峔n"
                                    for following in data:
                                        no += 1
                                        ret_ += "\n " + str(no) + ". " + str(following["login"])
                                    ret_ += "\n\n銆� Total {} User 銆�".format(str(len(data)))
                                    ret_ += "\n\nUsage : GithubFollowing {}|銆宯umber銆�".format(text)
                                    client.sendMessage(to,ret_)
                                elif "message" in data:
                                    client.sendMessage(to,"User not found")
                                elif data == []:
                                ...
                                if data != [] and "message" not in data:
                                    if int(cond[1]) <= len(data):
                                        ...
                                        ret_ += "\n\n銆� https://github.com/{} 銆�".format(following["login"])
...
#=====================================================================
                    ...
                    if settings["addImage"]["status"] == True and sender == clientMID:
                        ...
                        f = codecs.open("image.json","w","utf-8")
                        json.dump(images, f, sort_keys=True, indent=4, ensure_ascii=False)
                        client.sendMessage(to, " 銆� Picture 銆峔n 鈥� Detail: Add picture\n 鈥� Status: Succes added picture {} in list".format(str(settings["addImage"]["name"])))
                    ...
                    if settings["messageSticker"]["addStatus"] == True and sender == clientMID:
                        ...
                        if name != None and name in settings["messageSticker"]["listSticker"]:
                            ...
                            client.sendMessage(msg.to, " 銆� Sticker 銆峔nName: "+a.title+"\nSTKID: "+msg.contentMetadata['STKID']+"\nSTKPKGID: "+msg.contentMetadata['STKPKGID']+"\nSTKVER: "+msg.contentMetadata['STKVER'])
